...
app.get("/addstudent", (req, res) => {
    ...
    // Sanitization XSS...
    let qry = "select * from mern where email=? or number=?";
    mysql.query(qry, [email, phone], (err, results) => {
        if (err)
            ...
            if (results.length > 0) {
                ...
                // insert query
                let qry2 = "insert into mern values(?,?,?,?)";
...
app.get("/searchstudent", (req, res) => {
    ...
    let qry = "select * from mern where number=?";
...
app.get("/updatestudent", (req, res) => {
    ...
    let qry = "update mern set name=?, gender=? where number=?";
...
app.get("/removestudent", (req, res) => {
    ...
    let qry = "delete from mern where number=?";
    mysql.query(qry, [phone], (err, results) => {
        if (err) throw err
        else {
            if (results.affectedRows > 0) {
